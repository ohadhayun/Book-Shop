...
router.get('/', (req, res) => {
    booksModel.find({}, (err, documents) => {
        err ? res.status(500).send('error') : res.status(200).send(documents);
    })
});

router.get('/:id', (req, res) => {
    booksModel.find({ id: req.params.id }, (err, documents) => {
        err ? res.status(500).send('error') : res.status(200).send(documents);
    })
});
...
router.post('/add', (req, res) => {
    ...
    const dish = new booksModel(req.body);
    dish.save().then(() => res.send("success"));
});


router.get('/search/:searchedWord', (req, res) => {
    booksModel.find({
        $or: [
            { bookName: { "$regex": req.params.searchedWord, "$options": "i" } },
            { author: { "$regex": req.params.searchedWord, "$options": "i" } },
            { category: { "$regex": req.params.searchedWord, "$options": "i" } }
        ]
    }, (err, documents) => {
        if (err) {
